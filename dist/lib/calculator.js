import { metalAlloys, metals } from "./data";
const fieldLabels = {
...
const ensurePositive = (value, label, errors) => {
    if (!(value > 0)) {
        errors.push(`${label} должно быть больше нуля`);
    }
};
const getDensity = (metalId, alloyId) => {
...
export const calculateWeight = (input) => {
    const errors = [];
    const normalize = (value) => Number.isFinite(value !== null && value !== void 0 ? value : NaN) ? Math.abs(value) : 0;
    ...
    requiredByShape[input.shapeId].forEach((field) => {
        var _a;
        ensurePositive((_a = {
            width,
            height,
            ...
            quantity,
            length,
        }[field]) !== null && _a !== void 0 ? _a : 0, fieldLabels[field], errors);
    });
    if (errors.length) {
    ...
    switch (input.shapeId) {
        case 1: // Балка
            ...
            if (!errors.length) {
                volume =
                    ((width * height - (2 * (height - 2 * s) * (width - s2)) / 2) / 1000000) * length;
            }
            break;
        case 2: // Квадрат
            ...
            break;
        case 3: // Круг
            volume = ((Math.PI * diameter ** 2) / 4 / 1000000) * length;
            break;
        case 4: // Лист
            volume = (((width * height) / 1000000) * s * quantity) / 1000;
            break;
        case 5: // Полоса
            volume = ((width * length) / 1000000) * s;
            break;
        case 6: // Труба круглая
            if (diameter < s * 2) {
                ...
                volume = ((Math.PI * (diameter ** 2 - (diameter - s * 2) ** 2)) / 4 / 1000000) * length;
            }
            break;
        ...
        case 9: // Швеллер
            if (width < s * 2 || height < s) {
                errors.push(`Швеллер: ширина ≥ 2 × стенке и высота ≥ стенке (width=${width}, height=${height}, wall=${s})`);
            }
            else {
